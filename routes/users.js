...
router.delete("/:id", auth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Usuario deletado com sucesso!", deletedUser }); // Caso seja deletado, retornar mensagem e o usuario deletado
  } catch (error) {
    // Caso não funcione
...
router.put("/update/:id", auth, async (req, res) => {
  ...
  try {
    const updatedUser = await prisma.user.update({
      // Atualizando o usuario no DB
      where: { id: id }, // Onde o ID seja igual ao ID dos parametros
      data: updatedData, // Dados atualizados
    });
    res
      .status(200)
      .json({ message: "Usuario atualizado com sucesso!", updatedUser }); // Retornando mensagem de sucesso e o usuario atualizado
  } catch (error) {
    // Caso não funcione
    res.status(500).json({ error: "Erro no servidor!" }); // Retornando erro 500, erro no servidor
  }
...
router.post("/:id/avatar", auth, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao enviar imagem para o Cloudinary!" }); // Mensagem de erro no server 500
  }
});
